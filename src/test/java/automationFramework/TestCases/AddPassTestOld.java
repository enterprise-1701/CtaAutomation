...
public class AddPassTestOld {
	...
	public void addPass()throws ElementNotVisibleException, ElementNotFoundException, NoSuchElementException, Exception{
		...
		//If the element is not on the page it indicates card has reached maxium number of passes
		addPage.selectOneDayPass(driver);
		
		addPage.clickNextStep(driver);
		Utils.waitTime(3000);
		
		//Contiune if no exception 
		...
		Assert.assertEquals(cPage.getConfirmation(driver), "Thank you!");
			...
			Log.info("From the calling method - NoSuchElementException | ElementNotVisibleException caught");
			...
			AddPassSelectProductPage addPage2 = new AddPassSelectProductPage(driver);
			
			//AddPassSelectProductPage addPage = new AddPassSelectProductPage(driver);
			addPage2.selectOneDayPass(driver);
			
			addPage2.clickNextStep(driver);
			Utils.waitTime(3000);
	...
	//These two private methods are only called if exception is thrown 
	//existing card has reached the maxium number of passes
